import { initializeApp, cert } from 'firebase-admin/app'
import { getFirestore, FieldValue } from 'firebase-admin/firestore'

const app = initializeApp({
  credential: cert({
    projectId:   'crm-f8133',
    clientEmail: 'firebase-adminsdk-fbsvc@crm-f8133.iam.gserviceaccount.com',
    privateKey: `-----BEGIN PRIVATE KEY-----

wU6pONUAaUF5miTv7Hy+0eLJ
-----END PRIVATE KEY-----
`,
  }),
})

const db = getFirestore(app)
const ORG_ID = 'HzyDWOrkySUVdcoYUszi'

const snap = await db.collection(`organizations/${ORG_ID}/clients`).get()
let fixed = 0

for (const doc of snap.docs) {
  const data = doc.data()
  const phone = data.whatsappPhone || ''
  const hasLid = phone.includes('@lid')
  const hasSuffix = phone.includes('@s.whatsapp.net')

  if (hasLid || hasSuffix) {
    const cleanPhone = phone.replace('@s.whatsapp.net', '').replace('@lid', '')
    const jid = phone // guardar el JID original
    await doc.ref.update({
      whatsappPhone: cleanPhone,
      whatsappJid: jid,
      phone: data.phone?.includes('@') ? cleanPhone : data.phone,
      updatedAt: FieldValue.serverTimestamp(),
    })
    console.log(`✅ Arreglado: ${data.name} → phone: ${cleanPhone}, jid: ${jid}`)
    fixed++
  }
}

if (fixed === 0) console.log('No había clientes con @lid o @s.whatsapp.net')
else console.log(`\n${fixed} cliente(s) arreglado(s)`)

process.exit(0)
